(function () {
    ...
    $(document).on("sidebar_type", function (e) {
        if (typeof setting !== typeof undefined) {
            ...
            if (e.detail.value.length !== 0) {
                if (
                    e.detail.currentValue !== "sidebar-mini" &&
                    e.detail.currentValue !== ""
                ) {
                    if (
                        sidebarType.includes("sidebar-hover") &&
                        !e.detail.value.includes("sidebar-mini")
                    ) {
        ...
        if (liveCustomizerPannel !== null) {
            if (liveCustomizerPannel.classList.contains("show")) {
                if (
                    ...
                    e.target.closest(".btn-check") == null
                ) {
                    bootstrap.Offcanvas.getInstance(
                        liveCustomizerPannel
                    ).hide();
                }
            }
        }
    ...
    if (
        ...
        const liveCustomizerPannel = document.querySelector("#live-customizer");
        const liveCustomizerInstance = new bootstrap.Offcanvas(
            liveCustomizerPannel
        );
